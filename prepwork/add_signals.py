...
def aggregate_windows(main):

	tmp = main[0]

	for trades in main[1:]:

		tmp = tmp.merge(trades, on=['TTC', 'Drawdown', 'Datetime', 'Direction'], how='outer')

	tmp = tmp.fillna(0).sort_values('Datetime').reset_index(drop=True)

	return tmp[tmp.TTC > 0]
